...
const PostList = () => {
  ...
  if (status === "pending") return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-3 md:gap-x-8 md:gap-y-12 mt-4 md:mt-10">
      {[1, 2, 3, 4].map((n) => (
    ...
    <div className="relative">
      {allPosts.length === 0 ? (
        ...
        <InfiniteScroll
          ...
          className="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-3 md:gap-x-8 md:gap-y-16 mt-4 md:mt-12 pb-10 md:pb-20"
        >
          {allPosts.map((post) => (
            <div
              key={post._id}
